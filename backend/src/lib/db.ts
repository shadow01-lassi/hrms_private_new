...
export async function initDatabase(retries = 5, delay = 1000): Promise<void> {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            try {
                console.log("🌱 Seeding menu as per boss feedback...");

                // Clear previous menu data
                await pool.query(`DELETE FROM menu_master WHERE mm_name IN 
                  ('dashboard','masters','company-master','employee-master','reports','admin-tools','user-roles','login-credentials','menu-structure','settings')`);

                // Create main "Admin Tools" parent
                const { rows: adminTools } = await pool.query(`
                  INSERT INTO menu_master (mm_name, mm_label, mm_parent_id, mm_access_type, mm_link, mm_icon, mm_order)
                  VALUES ('admin-tools', 'Admin Tools', NULL, 'AD', NULL, 'Shield', 20)
                  RETURNING mm_id
                `);

                const adminToolsId = adminTools[0].mm_id;

                // Add items under Admin Tools
                await pool.query(`
                  ...
                  ('user-roles', 'User Roles', $1, 'AD', '/dashboard/admin-tools/user-roles', 'UserCog', 3),
                  ('menu-structure', 'Menu Structure', $1, 'AD', '/dashboard/admin-tools/menu-structure', 'LayoutGrid', 4),
                  ('login-credentials', 'Login Credentials', $1, 'AD', '/dashboard/registers/login-credentials', 'Key', 5)
                `, [adminToolsId]);

                // Add Dashboard and Reports at top level
                await pool.query(`
                  INSERT INTO menu_master (mm_name, mm_label, mm_parent_id, mm_access_type, mm_link, mm_icon, mm_order) VALUES 
                  ('dashboard', 'Dashboard', NULL, 'AD', '/dashboard', 'LayoutDashboard', 10),
                  ('reports', 'Reports', NULL, 'AD', '/dashboard/reports', 'FileText', 30),
                  ('settings', 'Settings', NULL, 'AD', '/dashboard/settings', 'Settings', 40)
                `);

                console.log("✅ Menu structure updated: Company Master is now under Admin Tools");
            } catch (menuErr) {
